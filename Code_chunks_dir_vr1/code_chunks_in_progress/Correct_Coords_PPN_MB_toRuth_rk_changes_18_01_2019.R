...
##Hi Ruth! Here are  my attempts to get the transect coordinates to work :

#Load the data: 
pops_2<-read.csv("fullPPN_dataset_2019-07-01_.csv",sep=",",header=T)

names(pops_2)
unique(pops_2$transect_Lon_start)

#Create a sub-dataframe with only one line per site 
# (to be able to spot general formattign mistakes)
...
which(grepl("\xa", pops_2$transect_Lat_start))
####  This issue does not appear with the new datafile - 2019-07-01_.csv 
####  so I can ignore it? ####


#### This section is no longer needed - RK
# pops_2[18394,]
...
dir()
sites <- read.csv("Coordinates_Feb2018_site_level.csv" )
sites[sites$site_code == "PC",]
...
### 
summary(droplevels(pops_2[pops_2$site_code == "PC",]))  ## only 1 transect at this site. 

### change coords based on online coordinate converter as follows.
names(pops_2)
pops_2[pops_2$site_code == "PC","transect_Lat_start"] <- "38.5177007"
pops_2[pops_2$site_code == "PC","transect_Lon_start"]  <- "-121.7643222"
pops_2[pops_2$site_code == "PC","transect_Lat_stop"]# <- "38.5176821"
pops_2[pops_2$site_code == "PC","transect_Lon_stop"]  <- "-121.7642651"
# 
