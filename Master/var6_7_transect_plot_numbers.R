...
warning("Errors. One site (HV) uses a,b,c,d (this site has 1m plots divided in 4 to make 4 50cm plots), P1-P9 and P01-P09, 
...
# ## will return a warning if levels are not correct
 ...
 if(any(issues1 != 0)) {
   print(site_checks)
   for(i in 1:length(site_checks)) {
     print(unique(pdata$transect_Lat_start[pdata$site_code == site_checks[i]]) )
   }
   }
