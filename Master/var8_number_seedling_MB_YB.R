...
for(p in 1:length(plots_names)){
  ...
  if(length(sld_numb)>1){#If not, then produce an error message
   # print(paste("plot ",plots_names[p],"has different seedling numbers written down"))
    ...
    seeds$number_seedlings[which(seeds$unique_plot_year_id==plots_names[p])]<-max(sld_numb)
    
    print(plots_names[p])#The number of the plot will appear just after the list of the plots for which an NA was replaced, which is what we want. 
  }
}

print("This is not a warning, the plots are flagged only for transparancy. The error was dealt with")

#####
print("Some number of seedlings might be outliers; they are not removed or changed in anyway, but flagged on the following plot")
seeds$number_seedlings
boxplot(seeds$number_seedlings,main="Outliers in number_seedlings?",ylab="number of seedlings")
q98<-quantile(seeds$number_seedlings, c(.98)) 
abline(h=q98,col="red")
par(xpd=T)
text(x=1.2,y=-20,"red line = 98% quantile",col="red")
text(x=1.3,y=max(seeds$number_seedlings,na.rm=T),paste("max =",max(seeds$number_seedlings,na.rm=T)),col="black")
text(x=0.8,y=-50,"The rest of the script dealt with 
      different number of seedlings per plot 
     by using the maximum value recorded for that plot.")
